...
app.post('/fms/sync-dibiaa', async (req, res) => {
    try {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (step1Act) plans[2] = addWorkdays(step1Act, 1); else if (I === 'No' && B!=='OTD' || B!=='Jewellery (OTD)') plans[2] = addWorkdays(A, 1); else if (step4Act) plans[2] = addWorkdays(step4Act, 1); //step2 plan
